<?php
include 'db.php';
session_start();

$kursus_id = $_GET['kursus_id'];
$message = '';
$msg_type = '';

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $judul = $_POST['judul'];
    $deskripsi = $_POST['deskripsi'];
    $link_embed = $_POST['link_embed'];

    try {
        $stmt = $pdo->prepare("INSERT INTO materi (kursus_id, judul, deskripsi, link_embed) VALUES (?, ?, ?, ?)");
        $stmt->execute([$kursus_id, $judul, $deskripsi, $link_embed]);

        // Set pesan notifikasi berhasil
        $_SESSION['message'] = 'Materi berhasil ditambahkan!';
        $_SESSION['msg_type'] = 'success';

    } catch (PDOException $e) {
        // Set pesan notifikasi gagal
        $_SESSION['message'] = 'Gagal menambahkan materi: ' . $e->getMessage();
        $_SESSION['msg_type'] = 'danger';
    }

    header('Location: index.php');
    exit();
}
?>

<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Tambah Materi</title>
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <div class="container">
        <h1>Tambah Materi</h1>
        <form method="post" action="">
            <div class="form-group">
                <label for="judul">Judul Materi</label>
                <input type="text" id="judul" name="judul" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="deskripsi">Deskripsi Materi</label>
                <textarea id="deskripsi" name="deskripsi" class="form-control" required></textarea>
            </div>
            <div class="form-group">
                <label for="link_embed">Link Embed Materi</label>
                <input type="text" id="link_embed" name="link_embed" class="form-control" required>
            </div>
            <button type="submit" class="btn btn-primary">Tambah Materi</button>
        </form>
    </div>
</body>
</html>
